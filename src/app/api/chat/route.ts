...
export async function GET() {
  try {
    ...
    console.error("[API] GET /api/chat failed:", err);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { message, genreSlug, imageBase64 } = parsed.data;

    // Sanitize user message — prevent prompt injection into AI system prompts
    ...
    // Build message array for Gemini — use sanitized message for current turn
    const messages = [
    ...
    // Save user message to DB (original for display, sanitized goes to AI)
    ...
    console.error("[API] POST /api/chat failed:", err);
...
export async function DELETE() {
  try {
    ...
    console.error("[API] DELETE /api/chat failed:", err);
